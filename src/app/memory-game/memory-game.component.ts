...
export class MemoryGameComponent implements OnInit {
  ...
  shuffleWord() {
    ...
    for (let i = this.cardGrid.length - 1; i > 0; i--) {
      ...
      [this.cardGrid[i], this.cardGrid[j]] = [
        this.cardGrid[j],
        this.cardGrid[i],
      ];
    }
  ...
  showCardWord(index: number): string {
    if (this.currentCategory?.words !== undefined) {
      if (this.cardGrid[index].isOrigin) {
        return this.currentCategory.words[this.cardGrid[index].categoryWordID]
          .origin;
      }
      return this.currentCategory.words[this.cardGrid[index].categoryWordID]
        .target;
  ...
  flipCardAndCheckMatch(cardIndex: number) {
    if (this.firstFlippedCard !== null) {
      ...
      if (
        this.cardGrid[cardIndex].categoryWordID ===
        this.firstFlippedCard.categoryWordID
      ) {
